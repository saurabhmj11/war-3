...
export const DEMO_ACTS: DemoAct[] = [
  {
    ...
    description:
      'MetLife Stadium opens its gates for the FIFA World Cup 2026. All 4 main turnstiles are processing fans smoothly with normal 18-minute wait times.',
  ...
  },
  {
    ...
    description:
      'Three NJ Transit commuter trains arrive simultaneously at Gate C East Plaza. Turnstile velocity surges to 340/min, causing wait times to spike to 42 minutes!',
  ...
  },
  {
    ...
    description:
      'Gemini analyzes concourse density and forecasts severe gridlock (>88% capacity) within 15 minutes, warning of potential spectator crush hazards.',
  ...
  },
  {
    ...
    description:
      'The What-If Simulation Engine evaluates opening Gate D auxiliary turnstiles and redirecting Sectors 101-115. Result: 35% congestion reduction in 12 minutes!',
  ...
  },
  {
    ...
    description:
      'Command center applies the simulation. Task 902 is dispatched instantly to volunteer Elena Rostova (Concourse B) with digital signage wands to direct crowd flow.',
  ...
  },
  {
    ...
    description:
      'The platform broadcasts targeted PA announcements and push notifications across 8 languages directing Gate C commuters to Gate D (5-min wait).',
  ...
  },
  {
    ...
    description:
      'A spectator collapses from heat exhaustion in Sector 112 row 14. A volunteer snaps a photo and submits an incident report via their mobile app.',
  ...
  },
  {
    ...
    description:
      'Medical Team Beta is routed via step-free Concourse B elevator. An 8-language PA alert instructs Sector 112 spectators to clear row 14 for stretcher access.',
  ...
  },
  {
    ...
    description:
      'Gate C wait time drops to 14 mins (GREEN). The medical patient is stabilized. Gemini 2.5 Pro generates a professional markdown executive report for FIFA match directors!',
    ...
    <div className="w-full bg-slate-900/95 border border-slate-700/80 rounded-3xl p-6 shadow-2xl flex flex-col gap-6 backdrop-blur-2xl">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-ping" aria-hidden="true" />
            <h2 className="text-xl font-extrabold text-white tracking-wide">9-Step Narrative Demo Command Center</h2>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Click through chronological acts to simulate a live 70,000-spectator World Cup match and watch AI Copilots adapt in real time. All state changes are synced to every connected browser tab via SSE.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold flex items-center gap-1.5 transition-all border border-slate-700 disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
          ...
          <button
            ...
            onClick={nextAct}
            disabled={isExecuting}
            className="px-5 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold flex items-center gap-2 shadow-lg shadow-emerald-900/40 transition-all disabled:opacity-50 active:scale-98 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400"
          >
            {isExecuting ? (
            ...
            <span>{currentAct === 9 ? 'Restart (Act 1)' : `Trigger Act ${currentAct + 1}`}</span>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
          <div className="space-y-3 max-w-2xl">
            ...
            <div className="pt-2 flex items-center gap-2 text-xs bg-slate-900/90 p-3 rounded-xl border border-slate-800 text-emerald-300">
              <Sparkles className="w-4 h-4 shrink-0 text-emerald-400" aria-hidden="true" />
              <div>
                <span className="font-bold text-white">Why Gemini is essential: </span>
          ...
          <div
            className="flex flex-col items-center justify-center bg-slate-900/80 border border-slate-800 p-5 rounded-2xl min-w-[160px] text-center shrink-0"
            role="status"
            aria-label={`Storyline act ${currentAct} of 9`}
          >
            <div className="text-3xl font-black text-white font-mono">
              {currentAct} <span className="text-slate-500">/</span> 9
            </div>
            <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">Storyline Act</div>
            <div className="mt-3 w-full bg-slate-800 rounded-full h-2 overflow-hidden" role="progressbar" aria-valuenow={currentAct} aria-valuemin={1} aria-valuemax={9}>
              <div
      ...
      <div className="space-y-2">
        <div className="text-xs font-bold uppercase tracking-wider text-slate-400">
          Chronological scenario roadmap (click any card to jump)
        </div>
          ...
          {DEMO_ACTS.map((actItem) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-start p-2.5 rounded-xl border text-left transition-all relative overflow-hidden group focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 disabled:opacity-50 ${
                  isCurrent
              ...
              >
                ...
                <div className="text-[10px] text-slate-400 truncate w-full mt-0.5">{actItem.subtitle}</div>
              </button>
            );
          })}
        </div>
        ...
        <div className="w-full bg-slate-950 border border-emerald-500/40 rounded-2xl p-6 shadow-2xl animate-in fade-in slide-in-from-top-4 duration-300">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
            <div className="flex items-center gap-2 text-emerald-400 font-bold text-sm">
              ...
              <span>Generated Executive Markdown Report</span>
            </div>
            <span
            ...
            >
              {lastEngine === 'gemini' ? 'GEMINI 2.5 PRO' : 'SIMULATED FALLBACK'}
            </span>
          </div>
          <div className="prose prose-invert prose-sm max-w-none text-slate-300 space-y-4 font-sans leading-relaxed">
            <div className="whitespace-pre-line bg-slate-900/80 p-4 rounded-xl border border-slate-800 font-mono text-xs max-h-96 overflow-y-auto">
              {lastSummary}
            </div>
          </div>
        </div>
