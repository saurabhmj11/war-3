        ...
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs font-bold uppercase tracking-wider text-purple-400 font-mono">PERSONA 5: DR. ARIS THORNE • PARAMEDIC FIRST-AID LEAD</span>
              <span className="text-[10px] bg-purple-500/20 text-purple-300 border border-purple-500/30 px-2 py-0.5 rounded-full font-mono">MED-STATION-1</span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white mt-1 flex items-center gap-2.5">
              <HeartPulse className="w-7 h-7 text-purple-400" aria-hidden="true" />
          ...
          <div className="lg:col-span-5 w-full space-y-6">
            <div className="bg-slate-900/90 border border-slate-800/80 rounded-3xl p-6 shadow-2xl backdrop-blur-xl space-y-4">
              ...
              <div className="space-y-4 max-h-[600px] overflow-y-auto pr-1">
                {incidents.length === 0 ? (
                  <div className="text-center py-8 text-xs text-slate-500">🟢 Zero active medical incidents. Medical stations operating at standby.</div>
                ) : (
                  incidents.map((inc) => {
                    const pri = getPriorityStyle(inc.severity);
                    return (
                      <div key={inc.incidentId} className="bg-slate-950/90 border border-purple-500/40 p-4 rounded-2xl space-y-3 shadow-lg">
                        <div className="flex items-center justify-between">
                          <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-md border ${pri.bg}`}>{pri.label}</span>
                        ...
                        <div className="pt-2 border-t border-slate-800/80 flex items-center justify-between gap-2">
                          ...
                          <button
                            ...
                            className="px-3 py-1.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold transition-all flex items-center gap-1.5 shadow-md active:scale-95 focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-400"
          ...
          <div className="lg:col-span-7 w-full space-y-6">
            <div className="bg-slate-900/90 border border-slate-800/80 rounded-3xl p-6 shadow-2xl backdrop-blur-xl space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                ...
                <span className="text-[10px] font-mono bg-cyan-500/20 text-cyan-300 border border-cyan-500/30 px-2 py-0.5 rounded-full font-bold">WCAG AA COMPLIANT • ZERO STAIRS</span>
              </div>
              <p className="text-xs text-slate-300">
                To prevent patient jarring during stretcher extraction, our routing filters out stairs and steep concourse ramps, calculating the fastest step-free service elevator path to Medical Station 1.
              </p>
              <div className="pt-2">
