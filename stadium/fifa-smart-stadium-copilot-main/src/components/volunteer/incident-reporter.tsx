...
export const IncidentReporter: React.FC = () => {
  ...
  useEffect(() => {
    const unsubInc = repository.subscribe<Incident>('incidents', (data) => setRecentIncidents(data));
    const unsubTasks = repository.subscribe<Task>('tasks', (data) => setTasks(data));
    return () => {
      unsubInc();
      unsubTasks();
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success && json.data) {
        setLastClassification(json.data.aiClassification);
        setDescription('');
        setPhotoUrl('');
      }
    } catch (err) {
  ...
  const handleTaskCheck = async (task: Task, itemIndex: number) => {
    ...
    await repository.updateTaskStatus(task.taskId, allDone ? 'COMPLETED' : 'IN_PROGRESS', updatedChecklist);
  };
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      {/* Left Column: Multimodal Incident Reporter (6 Cols) */}
        ...
        <div className="bg-slate-900/90 border border-slate-800/80 rounded-3xl p-6 shadow-2xl backdrop-blur-xl space-y-6">
          <div className="flex items-center justify-between border-b border-slate-800 pb-4">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center text-slate-950 font-bold shadow-lg shadow-blue-500/20">
          ...
          <div className="space-y-2">
            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider">
              Quick Test Scenarios (Simulate Camera Upload):
            </span>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {SAMPLE_PHOTOS.map((sample, idx) => (
                ...
                >
                  <div className="font-bold truncate group-hover:text-blue-400">{sample.label}</div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Sector / Concourse</label>
                <select
                  value={sector}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Photo URL (Optional)</label>
                <div className="relative">
                  <input
                    ...
                    placeholder="https://..."
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-8 pr-3 py-2.5 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500"
                  />
                  <ImageIcon className="w-3.5 h-3.5 text-slate-500 absolute left-2.5 top-3" />
                </div>
              </div>
            </div>
            ...
            {photoUrl && (
              <div className="relative rounded-2xl overflow-hidden border border-slate-800 max-h-40 bg-slate-950 flex items-center justify-center">
                ...
                <div className="absolute bottom-2 right-2 bg-slate-950/80 backdrop-blur-md px-2 py-1 rounded-lg text-[10px] text-blue-400 font-mono font-bold">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-400 uppercase mb-1.5">Incident Description</label>
              <textarea
                rows={3}
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="Describe what you see or transcribe radio report..."
                className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs sm:text-sm text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500"
              />
            </div>
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-500 hover:to-cyan-500 text-white font-extrabold text-xs shadow-lg shadow-blue-900/40 transition-all flex items-center justify-center gap-2 disabled:opacity-50 active:scale-98"
            >
              {isSubmitting ? (
          ...
          {lastClassification && (
            <div className="bg-gradient-to-r from-blue-950/60 to-cyan-950/60 border border-blue-500/40 rounded-2xl p-4 shadow-xl space-y-3 animate-in fade-in duration-300">
              <div className="flex items-center justify-between border-b border-blue-500/30 pb-2">
                <div className="flex items-center gap-2 text-xs font-bold text-blue-300">
                  ...
                  <span>Gemini Vision Classification Result</span>
                </div>
              ...
              <div className="text-[11px] text-emerald-400 font-bold flex items-center gap-1.5">
                ...
                <span>Ticket logged in Firestore & dispatched to {lastClassification.requiredTeam} triage!</span>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Right Column: Assigned Tasks & Live Incident Feed (6 Cols) */}
      <div className="lg:col-span-6 w-full space-y-6">
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-md ${
                      task.priority === 'URGENT' ? 'bg-red-500/20 text-red-300 border border-red-500/30' : 'bg-blue-500/20 text-blue-300'
                    }`}>
                  ...
                  <div className="mt-3 space-y-1.5 border-t border-slate-800/80 pt-3">
                    {task.checklist.map((item, idx) => (
                      <label
                        key={idx}
                        className="flex items-center gap-2.5 text-xs text-slate-300 cursor-pointer hover:text-white p-1.5 rounded-lg hover:bg-slate-900/60 transition-all"
                      >
                        <input
                          type="checkbox"
                          checked={item.completed}
                          onChange={() => handleTaskCheck(task, idx)}
                          className="w-4 h-4 rounded border-slate-700 text-blue-600 focus:ring-0 bg-slate-950"
                        />
        ...
        <div className="bg-slate-900/90 border border-slate-800/80 rounded-3xl p-6 shadow-2xl backdrop-blur-xl space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            ...
            <span className="text-xs text-slate-400 font-mono">FIRESTORE SYNC</span>
          </div>
              ...
              return (
                <div key={inc.incidentId} className="bg-slate-950/80 border border-slate-800/80 p-3.5 rounded-2xl space-y-2">
                  ...
                  <div className="text-xs font-bold text-white flex items-center gap-1.5">
                    {inc.incidentType === 'MEDICAL' && <HeartPulse className="w-3.5 h-3.5 text-purple-400" />}
                    {inc.incidentType === 'SECURITY' && <ShieldAlert className="w-3.5 h-3.5 text-red-400" />}
                    {inc.incidentType === 'CROWD_CONGESTION' && <Users className="w-3.5 h-3.5 text-amber-400" />}
                    {inc.incidentType === 'MAINTENANCE' && <Wrench className="w-3.5 h-3.5 text-cyan-400" />}
                    <span>{inc.incidentType}: {inc.description}</span>
                  </div>
                  {inc.aiSummary && (
                    <div className="text-[11px] text-slate-400 bg-slate-900 p-2 rounded-xl border border-slate-800 font-mono">
                      {inc.aiSummary}
                    </div>
                  )}
                </div>
