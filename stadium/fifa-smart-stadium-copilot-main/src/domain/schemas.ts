...
export const GateSchema = z.object({
  ...
  currentWaitMinutes: z.number().min(0),
  turnstileVelocityPerMin: z.number().min(0),
  maxCapacityPerMin: z.number().min(0),
  assignedSectors: z.array(z.string()),
});
...
export const CrowdMetricSchema = z.object({
  metricId: z.string().min(1),
  stadiumId: z.string().min(1),
  zoneId: z.string().min(1),
  timestamp: z.string(),
  currentDensityPct: z.number().min(0).max(100),
  riskLevel: RiskLevelSchema,
  predictedWaitMinutes: z.number().min(0),
  aiCongestionForecast: z.string().optional(),
});
...
export const TaskSchema = z.object({
  ...
  stadiumId: z.string().min(1),
  assignedToVolunteerId: z.string().min(1),
...
export const AnnouncementSchema = z.object({
  ...
  targetAudience: z.array(z.string()),
  priority: TaskPrioritySchema,
  status: z.enum(['DRAFT', 'BROADCASTED']),
  translations: z.record(LanguageCodeSchema, z.string()),
  broadcastedAt: z.string().optional(),
});
